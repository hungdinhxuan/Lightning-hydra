...
from src.data.components.collate_fn import multi_view_aux_collate_fn, variable_multi_view_collate_fn, ChunkingCollator
# augwrapper
...
class Dataset_for(Dataset_base):
    def __init__(self, args, list_IDs, labels, base_dir, algo=5, vocoders=[],
                 ...
                 aug_dir=None, online_aug=True, repeat_pad=True, is_train=True, random_start=False, aux_labels=None,
                 **kwargs):
...
class AuxDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        if self.is_variable_multi_view:
            ...
            self.collate_fn = lambda x: multi_view_aux_collate_fn(
                x,
                self.args.views,
                self.args.wav_samp_rate,
                self.args.padding_type,
