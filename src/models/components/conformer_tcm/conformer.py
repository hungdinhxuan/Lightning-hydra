...
from speechbrain.lobes.models.ECAPA_TDNN import Res2NetBlock, SEBlock
# helper functions
    ...
    def __init__(
        ...
        if type == 'conv':
            ...
            self.net = nn.Sequential(
                ...
                Swish(),
                nn.Conv1d(inner_dim, dim, 1),
                Res2NetBlock(dim, dim, scale=4, dilation=3),
                Rearrange('b c n -> b n c'),
                nn.Dropout(dropout))

        elif type == 'conv_seblock':
            self.net = nn.Sequential(
                nn.LayerNorm(dim),
                Rearrange('b n c -> b c n'),
                nn.Conv1d(dim, inner_dim * 2, 1),
        ...
        elif type == 'conv_res2net_seblock':
            self.net = nn.Sequential(
                nn.LayerNorm(dim),
                Rearrange('b n c -> b c n'),
                nn.Conv1d(dim, inner_dim * 2, 1),
...
class ConformerBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn = Attention(dim=dim, dim_head=dim_head,
                              heads=heads, dropout=attn_dropout)
        self.conv = ConformerConvModule(
            dim=dim, causal=conv_causal, expansion_factor=conv_expansion_factor, kernel_size=conv_kernel_size, dropout=conv_dropout, type=type)
