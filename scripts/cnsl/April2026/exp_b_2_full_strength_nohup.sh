#!/bin/bash
#
# EXP B: full-strength MBCT (full model), detached log + PID (trial-experiment-monitoring).
# Required: logger=wandb, +trainer.val_check_interval=0.5, ++model_averaging=True
# (~92k train utterances → align with 2_mbct_lora_full_strength_nohup.sh)
#
# Usage:
#   export XLSR_PRETRAINED_MODEL_PATH=pretrained/xlsr2_300m.pt
#   bash scripts/cnsl/April2026/exp_b_2_full_strength_nohup.sh
#   bash scripts/cnsl/April2026/exp_b_2_full_strength_nohup.sh -d 0
...
LOG_DIR="$REPO_ROOT/logs/train/exp_B_full_strength_mbct"
mkdir -p "$LOG_DIR"
...
nohup env \
  ...
  experiment=cnsl/April2026/exp_b_xlsr_conformertcm_mbct \
