#!/usr/bin/env python
"""script to create protocol for the Multilingual LibriSpeech (MLS) database
...
# ['$ROOT', 'MLS', 'mls_italian', 'train', 'audio', '1595', '5616', '1595_5616_000327.flac']
        subset = parts[3]
        if 'train' in subset:
            proportion = 'train'
        elif 'dev' in subset:
        ...
        label = 'real'
        attack = '-'
        file_id = f'{language}-{os.path.splitext(parts[-1])[0]}'
        try:
