...
CUDA_VISIBLE_DEVICES=$CUDA_DEVICE OMP_NUM_THREADS=5 python src/train.py \
    experiment=wildspoof/Oct31/xlsr_beats_seres2net_conformertcm_aux_fixed \
    ++data.data_dir="/data/Datasets/spoofceleb_spoof_noise_dataset" \
    ++data.args.protocol_path="data/Wild_Spoof_Dataset/spoofceleb_noise_dataset/protocol_aux2.txt" logger=wandb \
